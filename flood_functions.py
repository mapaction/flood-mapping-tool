...
def mask_permanent_water(image):
    """
    Query the JRC Global Surface Water Mapping Layers, v1.3, to determine where 
    perennial water bodies (water > 10 months/yr), and mask these areas.

    Inputs:
        image (ee.Image): Input image.

    Returns:
        masked_image (ee.Image): The resulting image after surface water masking 
        is applied.
    """
    ...
    # Flooded layer where perennial water bodies(water > 10 mo / yr) is assigned 
    # a 0 value
...
def reduce_noise(image):
    ...
    neighbours. This operation reduces noise of the flood extent product.
...
def mask_slopes(image):
    """
    Mask out areas with more than 5 percent slope using a Digital Elevation 
    Model.
...
def derive_flood_extents(
    ...
    """
    Set start and end dates of a period BEFORE and AFTER a flood. These periods 
    need to be long enough for Sentinel-1 to acquire an image.
    ...
    Inputs:
        ...
        difference_threshold (float): Threshold to be applied on the differenced 
            image (after flood - before flood). It has been chosen by trial and 
            error. In case your flood extent result shows many false-positive or 
            negative signals, consider changing it.
